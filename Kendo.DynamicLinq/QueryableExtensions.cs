    ...
    public static class QueryableExtensions
    {
        ...
        private static IQueryable<T> Filter<T>(IQueryable<T> queryable, Filter filter)
        {
            if (filter != null && filter.Logic != null)
            {
                ...
                List<object> preValue = new List<object>();

                foreach (var item in filters)
                {
                    if (item.Field == "CreationTime" || item.Field == "LastModificationTime")
                    {
                        preValue.Add((object)(DateTime.Parse(item.Value.ToString())));
                    }
                    else
                    {
                ...
                //    var temptest = filters.Select(f => (object)(DateTime.Parse(f.Value.ToString()))  ).ToArray();
                //        queryable = queryable.Where(predicate, temptest);
                //}
                //else
                //{

                values = preValue.ToArray();  //filters.Select(f => f.Value).ToArray();
                ...
                //if (predicate.Contains("CreationTime") || predicate.Contains("LastModificationTime"))
                //{
                //    var temp = values.Select(x => new  { DateTime.Parse(x.ToString()) }).ToArray();
                //    queryable = queryable.Where(predicate, temp);
                //}
                //else
                //{
                // }
        ...
        //private static IQueryable<T> Filter<T>(IQueryable<T> queryable, IEnumerable<Filter> filters)
        //{
        //    if (filters != null)
        //    {
        ...
        //            {
        //                // Collect a flat list of all filters
        //                var _filters = filter.All();

        //                // Get all filter values as array (needed by the Where method of Dynamic Linq)
        //                var values = _filters.Select(f => f.Value).ToArray();

        //                // Create a predicate expression e.g. Field1 = @0 And Field2 > @1
        //                string predicate = filter.ToExpression(_filters);
